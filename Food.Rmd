...
This is a self reported data set about college students. It includes items such as  GPA, life style choices, personal preferences, and family background. 

Many individuals and couples feel they must complete their education before they begin having children. I want to test if the amount of schooling a mother has affects a child's GPA. Thus the question:

Does a child's GPA change with the mother's level of education?

NULL:all means equal 
ALT : NOT equal
#how do I change the width?
```{r}
pander(kruskal.test(GPA ~ mother_education, data=food))

food$mother_education_char <- case_when(food$mother_education == 1 ~ "< highschool",
                                   food$mother_education == 2 ~ "highschool grad",
                                   food$mother_education == 3 ~ "some college",
                                   food$mother_education == 4 ~ "college grad", 
                                   food$mother_education == 5 ~ "graduate degree")  
                                     
food$mother_education_char <- factor(food$mother_education_char, levels=c(
      "< highschool",
      "highschool grad",
      "some college",
      "college grad", 
...
boxplot(GPA~mother_education_char, data=food, main = "Child's GPA Based on Mother's Level of Education", xlab = "Mother's Education In Years")

favstats(GPA~mother_education, data=food)[,-10] %>% 
  pander(caption = "Rank summary ")
```
This box plot shows there is no difference. 
...
Results: Fail to regect the null hythothisis,in other words I can't assume all the means are equal, because I have a 43.46% chance of being wrong. 